...
QuadTree::QuadTree()
	...
	for (size_t i = 0; i < MAX_CAPACITY; i++) this->p[i] = nullptr;
}
...
void QuadTree::calcCenterOfMass(void) {
	...
	if (this->count != 0) this->massCenter /= this->count;
}
...
void QuadTree::setMassCenter(const Vector2 &v) {
	this->massCenter = v;
}

void QuadTree::setTotalMass(const double_t &m) {
	this->totalMass = m;
}

void QuadTree::setSector(const u_int8_t &sector) {
	this->sector = sector;
}
...
const double_t &QuadTree::getTotalMass(void) const {
	return (this->totalMass);
}

const u_int8_t &QuadTree::getSector(void) const {
	return (this->sector);
}
...
Particle *const *QuadTree::getParticles(void) const {
	return (this->p);
}

void QuadTree::setParent(QuadTree *node) {
	this->parent = node;
}

QuadTree *QuadTree::getParent(void) const {
	return (this->parent);
}

void QuadTree::setChildren(QuadTree *node, const uint8_t &sector) {
	if (sector & NORTH_WEST) this->nw = node;
	if (sector & NORTH_EAST) this->ne = node;
	if (sector & SOUTH_WEST) this->sw = node;
	if (sector & SOUTH_EAST) this->se = node;
}

QuadTree *QuadTree::getChildren(const uint8_t &sector) const {
	if (sector & NORTH_WEST) return (this->nw);
	if (sector & NORTH_EAST) return (this->ne);
	if (sector & SOUTH_WEST) return (this->sw);
	if (sector & SOUTH_EAST) return (this->se);
	return (nullptr);
}
